import { memo } from 'react';
import type { FC } from 'react';

import resets from '../_resets.module.css';
import { Frame48096383Icon } from './Frame48096383Icon';
import { Frame48096384Icon } from './Frame48096384Icon';
import { Frame48096385Icon } from './Frame48096385Icon';
import { FrameIcon } from './FrameIcon';
import { FrameIcon2 } from './FrameIcon2';
import { FrameIcon3 } from './FrameIcon3';
import { IconIcon } from './IconIcon';
import { IconIcon2 } from './IconIcon2';
import classes from './LandingPage.module.css';

interface Props {
  className?: string;
}
/* @figmaId 6:562 */
export const LandingPage: FC<Props> = memo(function LandingPage(props = {}) {
  return (
    <div className={`${resets.storybrainResets} ${classes.root}`}>
      <div className={classes.frame14257}>
        <div className={classes.group}>
          <div className={classes.frame}>
            <div className={classes.frame2}>
              <div className={classes.frame3}>
                <div className={classes.frame4}>
                  <div className={classes.textblock}>
                    <div className={classes.frame5}>
                      <div className={classes.frame6}>
                        <FrameIcon className={classes.icon} />
                      </div>
                    </div>
                  </div>
                </div>
                <div className={classes.frame7}>
                  <div className={classes.icon2}>
                    <IconIcon className={classes.icon3} />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={classes.navBarOptions}>
          <div className={classes.home}>Home</div>
          <div className={classes.portal}>Portal</div>
          <div className={classes.howItWorks}>How it Works</div>
          <div className={classes.frame8}>
            <FrameIcon2 className={classes.icon4} />
          </div>
        </div>
      </div>
      <div className={classes.frame14258}>
        <div className={classes.frame14260}>
          <div className={classes.frame14333}>
            <div className={classes.frame14332}>
              <div className={classes.simplifyingMaterialTesting}>Simplifying Material Testing</div>
              <div className={classes.streamlineYourConstructionMate}>
                <div className={classes.textBlock}>
                  Streamline your construction material testing process with Honetus’ reliable and standardized
                  services, connecting you with top-quality labs in India.
                </div>
                <div className={classes.textBlock2}>
                  <p></p>
                </div>
              </div>
            </div>
          </div>
          <div className={classes.image24}></div>
        </div>
      </div>
      <div className={classes.frame14277}>
        <div className={classes.whyDoWeExist}>Why do we exist?</div>
        <div className={classes.frame142772}>
          <div className={classes.frame14271}>
            <div className={classes.image26}></div>
            <div className={classes.frame14270}>
              <div className={classes.reliableMaterialTestingPartner}>Reliable Material Testing Partner Connecter</div>
              <div className={classes.honestusPartnersWithQualityDri}>
                Honestus partners with quality-driven service providers who adhere to strict standards, ensuring
                standardized and reliable services for a seamless construction process.
              </div>
            </div>
          </div>
          <div className={classes.frame14273}>
            <div className={classes.image25}></div>
            <div className={classes.frame14272}>
              <div className={classes.streamlinedLogisticsSupportFor}>
                Streamlined Logistics Support for Construction Projects
              </div>
              <div className={classes.ourLogisticsSupportServiceEnsu}>
                Our logistics support service ensures that the right material testing lab is available when needed,
                saving clients time and reducing stress.
              </div>
            </div>
          </div>
          <div className={classes.frame14275}>
            <div className={classes.image27}></div>
            <div className={classes.frame14274}>
              <div className={classes.effortlessMaterialTestingLabCo}>Effortless Material Testing Lab Connections</div>
              <div className={classes.honestusStreamlinedSearchFunct}>
                Honestus’ streamlined search function simplifies the process of finding the right material testing lab
                by allowing users to filter by location, services
              </div>
            </div>
          </div>
          <div className={classes.frame14276}>
            <div className={classes.image33}></div>
            <div className={classes.frame142742}>
              <div className={classes.transparentTestingProcess}>Transparent Testing Process</div>
              <div className={classes.honestusOffersRealTimeProgress}>
                Honestus offers real-time progress monitoring and remote test witnessing for full transparency and
                accuracy.
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className={classes.frame14287}>
        <div className={classes.benefits}>Benefits</div>
        <div className={classes.frame14285}>
          <div className={classes.image28}></div>
          <div className={classes.frame14284}>
            <div className={classes.frame14280}>
              <div className={classes.ensuresStandardizedAndReliable}>
                Ensures standardized and reliable services for construction companies.
              </div>
            </div>
            <div className={classes.frame14279}>
              <div className={classes.eliminatesTheHassleOfFindingRe}>
                Eliminates the hassle of finding reliable material testing labs in India.
              </div>
            </div>
            <div className={classes.frame14282}>
              <div className={classes.increasesEfficiencyInTheConstr}>
                Increases efficiency in the construction industry by connecting the right partners for each project
              </div>
            </div>
            <div className={classes.frame14281}>
              <div className={classes.savesTimeAndResourcesByTakingC}>
                Saves time and resources by taking care of logistics
              </div>
            </div>
            <div className={classes.frame14283}>
              <div className={classes.promotesQualityAndReliabilityB}>
                Promotes quality and reliability by working with partners who meet high standards.
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className={classes.frame14269}>
        <div className={classes.frame14265}>
          <div className={classes.frame14264}>
            <div className={classes.frame14262}>
              <div className={classes._10}>10+</div>
              <div className={classes.expertPartnerLabsAcrossIndia}>Expert Partner Labs across India</div>
            </div>
            <div className={classes.frame14263}>
              <div className={classes.trustedByNumerousConstructionC}>Trusted by numerous construction companies</div>
            </div>
          </div>
          <div className={classes.frame48096659}>
            <div className={classes.honestusMadeOurProjectSeamless}>
              Honestus made our project seamless by connecting us with a reliable material testing lab and taking care
              of all the logistics.
            </div>
            <div className={classes.pratikSinha}>Pratik Sinha</div>
          </div>
        </div>
        <div className={classes.frame14268}>
          <div className={classes.a}>A+</div>
          <div className={classes.frame14267}>
            <div className={classes.frame14266}>
              <div className={classes.unnamed}>★</div>
              <div className={classes.unnamed2}>★</div>
              <div className={classes.unnamed3}>★</div>
              <div className={classes.unnamed4}>★</div>
              <div className={classes.unnamed5}>★</div>
            </div>
          </div>
        </div>
      </div>
      <div className={classes.frame14286}>
        <div className={classes.features}>Features</div>
        <div className={classes.frame142852}>
          <div className={classes.frame142842}>
            <div className={classes.frame142792}>
              <div className={classes.logisticsManagementForMaterial}>
                Logistics management for material testing labs
              </div>
            </div>
            <div className={classes.frame142802}>
              <div className={classes.standardizedAndReliableService}>Standardized and reliable services</div>
            </div>
            <div className={classes.frame142812}>
              <div className={classes.partnershipsWithHighQualityAnd}>
                Partnerships with high-quality and reliable testing labs
              </div>
            </div>
            <div className={classes.frame142822}>
              <div className={classes.efficientAndStreamlinedProcess}>
                Efficient and streamlined process for connecting construction companies with testing labs
              </div>
            </div>
            <div className={classes.frame142832}>
              <div className={classes.simplifiedProcessForMaterialTe}>
                Simplified process for material testing in the construction industry
              </div>
            </div>
          </div>
          <div className={classes.image282}></div>
        </div>
      </div>
      <div className={classes.frame14290}>
        <div className={classes.frame14289}>
          <div className={classes.frame14288}>
            <div className={classes.partnerWithUs}>Partner with Us</div>
            <div className={classes.ourCommitmentToHighStandardsOf}>
              <div className={classes.textBlock3}>
                Our commitment to high standards of quality and reliability in partnering with material testing labs
                simplifies the construction process and earns the trust of our clients.
              </div>
              <div className={classes.textBlock4}>
                <p></p>
              </div>
            </div>
          </div>
          <div className={classes.frame14301}>
            <div className={classes.joinOurPartnerLabNetwork}>Join our Partner Lab Network</div>
          </div>
        </div>
        <div className={classes.image32}></div>
      </div>
      <div className={classes.frame48096658}>
        <div className={classes.frame48096387}>
          <div className={classes.frame142572}>
            <div className={classes.group2}>
              <div className={classes.frame9}>
                <div className={classes.frame10}>
                  <div className={classes.frame11}>
                    <div className={classes.frame12}>
                      <div className={classes.textblock2}>
                        <div className={classes.frame13}>
                          <div className={classes.frame14}>
                            <FrameIcon3 className={classes.icon5} />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className={classes.frame15}>
                      <div className={classes.icon6}>
                        <IconIcon2 className={classes.icon7} />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={classes.frame48096386}>
            <div className={classes.frame48096383}>
              <Frame48096383Icon className={classes.icon8} />
            </div>
            <div className={classes.frame48096384}>
              <Frame48096384Icon className={classes.icon9} />
            </div>
            <div className={classes.frame48096385}>
              <Frame48096385Icon className={classes.icon10} />
            </div>
          </div>
        </div>
        <div className={classes.frame48096380}>
          <div className={classes.help}>Help</div>
          <div className={classes.frame48096379}>
            <div className={classes.privacyPolicy}>Privacy Policy</div>
            <div className={classes.shippingDelivery}>Shipping &amp; Delivery</div>
            <div className={classes.refundPolicy}>Refund Policy</div>
            <div className={classes.trackYourOrder}>Track Your Order</div>
          </div>
        </div>
        <div className={classes.frame48096381}>
          <div className={classes.importantLinks}>Important Links</div>
          <div className={classes.frame480963792}>
            <div className={classes.testingPartnerConnector}>Testing Partner Connector</div>
            <div className={classes.orderANewTest}>Order a New Test</div>
            <div className={classes.joinAsAPartnerLab}>Join as a Partner Lab</div>
          </div>
        </div>
        <div className={classes.frame48096382}>
          <div className={classes.support}>Support</div>
          <div className={classes.frame480963793}>
            <div className={classes.feedback}>Feedback</div>
            <div className={classes.contactUs}>Contact us</div>
            <div className={classes.customerService}>Customer Service</div>
            <div className={classes.termsConditions}>Terms &amp; Conditions</div>
          </div>
        </div>
      </div>
    </div>
  );
});
